...
def get_all_compliances_for_dropdown(request):
    ...
    for compliance in Compliance.objects.order_by('id').all():
        try:
            type_obj = ComplianceType.objects.get(pk = compliance.compliance_type_id)
            type = type_obj.name
        except:
            type = ''
        details = str(compliance.id) + ' - ' + compliance.description
        data.append({'id': compliance.id, 'name': compliance.abbrv, 'type': type, 'version': compliance.version_number, 'details': details})
...
def get_compliances_with_type(request, type_id):
    ...
    data = []
    try:
        compliances = Compliance.objects.filter(compliance_type_id = int(type_id))
    except:
        compliances = []
    for compliance in compliances:
        data.append({'id': compliance.id, 'name': compliance.abbrv, 'type_id': compliance.compliance_type_id, 'version': compliance.version_number})
...
def get_compliance_requirements_with_name(request, compliance_id):
    ...
    data = []
    try:
        requirements = ComplianceRequirement.objects.filter(compliance_id = int(compliance_id))
    except:
        requirements = []
...
def get_compliance_version_number(request, compliance_id):
    ...
    try:
        version = ComplianceVersion.objects.get(compliance_id = int(compliance_id))
    except:
        version = []
    if version == []:
        data = []
